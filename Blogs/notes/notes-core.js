/**
 * Notes core — shared by every lecture-notes series.
 *
 * Derived from deep-gen/dgm-notes.js on 18 Aug 2026 and kept behaviour-identical:
 * the render pipeline, the math placeholder dance, the TOC builder and the
 * cross-reference previews are the parts that took the longest to get right, so
 * they are not reimplemented here. Series-specific styling lives in the series'
 * own stylesheet; this file assumes only the shared class names.
...
(function () {
    ...
    function initRefPreviews() {
        ...
        function sectionSlice(md, n) {
            ...
            body = body.replace(/<div class="[a-z-]*widget"[^>]*><\/div>/g, '')
